...
public class ForecastBudgetMonth extends AppCompatActivity {
    DatabaseHelper myDb;
    SQLiteDatabase db;
    ...
    private Button btnForecastNextWeek;
    private TextView textViewForecastRangeDate;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.forecast_budget_month);
        openDatabase();

        listViewForecastList = (ListView)findViewById(R.id.listViewForecastList);
        btnForecastNextWeek = (Button)findViewById(R.id.btnForecastNextWeek);
        textViewForecastRangeDate = (TextView)findViewById(R.id.textViewForecastRangeDate);
        btnForecastNextWeek.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(ForecastBudgetMonth.this, ForecastBudgetWeek.class);
    ...
    public void showForecastExpense() {
        ...
        Cursor cur = db.rawQuery("SELECT *, EXPENSE.CategoryName, " +
                "INCOME.ID AS _id, " +
                "EXPENSE.ID AS _id, " +
                "ROUND((SUM(EXPENSE.ExpenseAmount)) * 4.33, 2) AS ExpenseForecast " +
        ...
        adapter = new SimpleCursorAdapter(ForecastBudgetMonth.this, R.layout.forecast_budget_row, cur, columns, to, 0);
